...
def my_town_name(town_name):
    """ tests if parameter (strings) match
    """
    if town_name.lower() == "westfield":
        # catch upper case entries
        return True
    else:
        return False 

my_town_name("Westfield")
...
concatenate_name("Erika", "Kettleson")
...
def getting_to_know_you(home_town, first_name, last_name):
    ...
    if my_town_name(home_town) == True:
        print "Hi {} Wow we're from the same place! CRAZY!".format(full_name)
    else:
        print "Well, {}, where are you from?".format(full_name)


getting_to_know_you("Westfield", "Erika", "Kettleson")
